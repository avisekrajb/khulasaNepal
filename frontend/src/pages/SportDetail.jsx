  ...
  const fetchArticleAndSuggestions = async () => {
    try {
      ...
      const [articleResponse, suggestionsResponse, mixedResponse] = await Promise.all([
        ...
        fetch(`${API_URL}/api/news/mixed-feed/${id}?limit=28`)
      ]);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <CategoryBannerAd position="category-banner" page="article" category="sports" />
              ...
              <div className="flex gap-8">
                ...
                <aside className="hidden xl:block w-72 sidebar-ads">
                  <div className="sticky top-28 space-y-4">
                    ...
                    {/* Related Sports - Show as fallback */}
                    {suggestedArticles.length > 0 && mixedNews.length <= 21 && (
                      <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg shadow-lg p-4">
                        ...
                        <div className="space-y-3">
                          {suggestedArticles.slice(0, 7).map((item, idx) => (
                            <div
                              key={item.id}
                              onClick={() => navigate(`/sports/${item.id}`)}
