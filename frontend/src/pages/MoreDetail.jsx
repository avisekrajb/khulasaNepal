    ...
    return (
      <div className={`min-h-screen ${isDark ? 'bg-gray-900' : 'bg-gray-50'} flex items-center justify-center`}>
        <div className="text-center">
          ...
          <p className={`${isDark ? 'text-gray-400' : 'text-gray-600'} text-lg`}>लोड गर्दै...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className={`min-h-screen ${isDark ? 'bg-gray-900' : 'bg-gray-50'} flex items-center justify-center`}>
        <div className="text-center max-w-md mx-auto px-4">
          ...
          <h2 className={`text-2xl font-bold ${isDark ? 'text-gray-100' : 'text-gray-800'} mb-4`}>समाचार भेटिएन</h2>
          <p className={`${isDark ? 'text-gray-400' : 'text-gray-600'} mb-6`}>{error || 'यो समाचार अवस्थित छैन वा हटाइएको छ।'}</p>
          <button
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <CategoryBannerAd position="category-banner" page="article" category="more" />
          ...
          <div className="lg:col-span-2">
            <article className={`${isDark ? 'bg-gray-800' : 'bg-white'} rounded-2xl shadow-lg overflow-hidden`}>
              ...
              <div className="flex items-center gap-4 mb-5 mt-5 mx-auto w-fit">
                <span className={`${isDark ? 'text-gray-300' : 'text-gray-600'} font-medium`}>सेयर:</span>
                <button
                ...
                <button
                  ...
                  className={`p-2 ${isDark ? 'bg-gray-700' : 'bg-gray-900'} text-white rounded-lg`}
                ...
                <button
                  ...
                  className={`p-2 ${isDark ? 'bg-gray-700 text-white hover:bg-gray-600' : 'bg-gray-300 text-black-950 hover:bg-gray-600'} transition rounded-lg`}
                  title="share"
                >
                  <Share size={20}/>
                </button>
              ...
              <div className="p-8">
                {/* Title */}
                <h1 className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-gray-900'} mb-4 leading-normal md:text-4xl`}>
                  {article.title}
                </h1>

                {/* Subtitle */}
                {article.subtitle && (
                  <p className={`text-xl ${isDark ? 'text-gray-300' : 'text-gray-600'} mb-6 leading-relaxed`}>
                    {article.subtitle}
                  </p>
                )}

                {/* Meta Information */}
                <div className={`flex flex-wrap items-center gap-6 pb-6 mb-6 border-b ${isDark ? 'border-gray-700' : 'border-gray-200'} mx-auto w-fit`}>
                  {/* Author Info */}
                  <div className="flex items-center gap-3">
                    ...
                    />
                    <div>
                      <p className={isDark ? 'text-white' : 'text-gray-900'}>{article.journalistName || 'खुलासा नेपाल'}</p>
                    </div>
                  </div>
                ...
                {article.paragraph && (
                  <div 
                    className={`prose prose-lg max-w-none ${isDark ? 'text-gray-300' : 'text-gray-700'} leading-relaxed p-3`}
            ...
            <div className="sticky lg:top-24 space-y-6">
              ...
              {relatedArticles.length > 0 && (
                <div className={`${isDark ? 'bg-gray-800' : 'bg-white'} rounded-2xl shadow-lg p-6`}>
                  <h3 className={`text-2xl font-bold mb-6 ${isDark ? 'text-white' : 'text-gray-900'}`}>सम्बन्धित समाचार</h3>
                    ...
                    {relatedArticles.map((item) => (
                      ...
                      >
                        ...
                        <div className="flex-1">
                          <p className={`font-semibold ${isDark ? 'text-white' : 'text-gray-900'} group-hover:text-blue-600 transition line-clamp-2`}>
                            {item.title}
                          </p>
                          <span className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'} mt-1 block`}>
          ...
          <div className="mt-16">
            <section className={`${isDark ? 'bg-gray-800' : 'bg-white'} rounded-2xl shadow-lg p-6 md:p-8`}>
              <div className="flex gap-8">
                {/* Main Content */}
                <div className="flex-1">
                  <h2 className={`text-3xl font-bold ${isDark ? 'text-white' : 'text-gray-900'} mb-8 pb-3 border-b-2 border-blue-600`}>थप समाचारहरू</h2>
                  ...
                  <div className="hidden xl:grid grid-cols-3 gap-6">
                    {mixedNews.slice(0, 21).map((item) => (
                      ...
                      >
                        ...
                        <div className="p-4">
                          <h3 className={`font-bold ${isDark ? 'text-white' : 'text-gray-900'} group-hover:text-blue-600 transition line-clamp-3 mb-2`}>
                            {item.title}
                          </h3>
                          {item.subtitle && (
                            <p className={`text-sm ${isDark ? 'text-gray-300' : 'text-gray-600'} line-clamp-2 mb-3`}>
                              {item.subtitle}
                            </p>
                          )}
                          <span className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>{getTimeAgo(item.publishedDate)}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 xl:hidden">
                    ...
                    {/* First Ad */}
                    <div className={`md:col-span-2 ${isDark ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-sm overflow-hidden`}>
                      <ArticleInlineAd 
                  ...
                  <div className="sticky top-28 space-y-4">
                    ...
                    {mixedNews.length > 21 && !allAdsLoaded && (
                      <div className={`${isDark ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-lg p-4`}>
                        <h3 className={`text-lg font-bold mb-4 ${isDark ? 'text-white' : 'text-gray-900'} pb-2 border-b ${isDark ? 'border-gray-700' : 'border-gray-200'}`}>
                          अन्य समाचार
                        </h3>
                        <div className="space-y-3">
                          {mixedNews.slice(21, 28).map((item) => (
                            ...
                            >
                              <p className={`text-sm font-semibold ${isDark ? 'text-white' : 'text-gray-900'} group-hover:text-blue-600 transition line-clamp-3 leading-relaxed`}>
                                {item.title}
                              </p>
                              <span className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'} mt-1 block`}>
                                {getTimeAgo(item.publishedDate)}
                              </span>
                            </div>
                          ))}
                    ...
                    {/* Related Articles - Show as fallback */}
                    {relatedArticles.length > 0 && mixedNews.length <= 21 && (
                      <div className={`${isDark ? 'bg-gray-800' : 'bg-gradient-to-br from-blue-50 to-indigo-50'} rounded-lg shadow-lg p-4`}>
                        <h3 className={`text-lg font-bold mb-4 ${isDark ? 'text-white' : 'text-gray-900'} pb-2 border-b ${isDark ? 'border-gray-700' : 'border-blue-200'}`}>
                          सम्बन्धित समाचार
                        </h3>
                        <div className="space-y-3">
                          {relatedArticles.slice(0, 7).map((item, idx) => (
                            ...
                            >
                              <div className="flex gap-2">
                                ...
                                <p className={`text-sm font-semibold ${isDark ? 'text-white' : 'text-gray-900'} group-hover:text-blue-600 transition line-clamp-2`}>
                                  {item.title}
                                </p>
                              </div>
                            </div>
